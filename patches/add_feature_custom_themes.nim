...
var __cdb_durFlare=__cdb_spinDurOk?__cdb_spinDurRaw:"2.4s";
...
// flare: a slow scale+glow throb (sun/ember flaring up and settling), distinct from
// the flat opacity throb of `pulse` -- built for `chai`'s sun glyph but usable by any
// theme via {"animation":"flare"}. The glow reads off currentColor (the accent).
__cdb_css+="@keyframes cdbFlare{0%,100%{transform:scale(1);filter:drop-shadow(0 0 0 currentColor)}50%{transform:scale(1.14);filter:drop-shadow(0 0 7px currentColor)}}";
...
__cdb_css+="svg[data-cdb-spinner].cdb-anim-flare{animation:cdbFlare "+__cdb_durFlare+" ease-in-out infinite;transform-origin:50% 50%;transform-box:fill-box}";
